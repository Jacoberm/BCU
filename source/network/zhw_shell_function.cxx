...
int VerifyFile(dfu_info_t dfu_info)
{
	...
	while( total_read_size < image_file_size )
	{
		...
		{
			if( total_read_size > image_file_size - 300*1024 || image_file_size < 300*1024 ) 
			{
				break;
			}
			else 
...
void DFUProc(network_shell_package_t t)
{
	...
	diag_printf("dev_name = %s\n",network_shell_package.dfu_info.dev_name);
	diag_printf("sub_cmd = %d\n",network_shell_package.dfu_info.sub_cmd);

	switch(network_shell_package.dfu_info.sub_cmd)
  	{
  	case GV:
		strncpy(network_shell_package.dfu_info.file_ver,
			bcu_state.pa_software_version.software_version,30);
		break;
...
unsigned char soft_count_zo = 0;
extern unsigned char enable_manual_auto_test ;
void ZhwBakFunc9(network_shell_package_t t)
{
	diag_printf("this is the ZhwBakFunc9() shell handle function!!!\n");
	if( soft_count_zo==0 ) {
	    enable_manual_auto_test = 1;
	    soft_count_zo = 1;
	}
	else
	{
	    soft_count_zo = 0;
	    enable_manual_auto_test = 0;
	}
