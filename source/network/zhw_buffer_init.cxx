...
int ZhwBufferRegister(int common_type,unsigned int pkg_type,unsigned int buffer_type, int buffer_id)
{
       ...
       diag_printf("head_buffer_id %d\n " ,buffer_info_head.buffer_id);

    while(tmp_buffer_info_point->next != NULL)
	{
		diag_printf("list_buffer_id %d   buffer_id %d\n",tmp_buffer_info_point->buffer_id,buffer_id);
		tmp_buffer_info_point = tmp_buffer_info_point->next;
	}

	tmp_buffer_info_point = &buffer_info_head;

	while(tmp_buffer_info_point->next != NULL)
	{
		if(tmp_buffer_info_point->buffer_id == buffer_id)
		{
			net_debug_printf2(("run out ZhwBufferInit() function the new id already in the list\n"));
			//return -1;
		}
		tmp_buffer_info_point = tmp_buffer_info_point->next;
	}
