...
///<BCU发送给其他设备的命令包
common_package_t dev_vol_to_eamp,dev_vol_to_pcu,dev_vol_to_bcu,recv_dev_col_bcu,dev_vol_to_ccu,monitor_ctrl_info;
...
/*station information*/
station_information_t station_information[2][MAX_STATION_NUMBER];///<这个只用作越战信息了，有点浪费
...
timeout_entry timeouts[NTIMEOUTS];
cyg_handle_t timeout_alarm_handle;
cyg_alarm timeout_alarm;
cyg_int32 last_delta;


unsigned short CRC16(char *CRC_Buf,unsigned int CRC_Leni) 
{ 
	unsigned int i,j; 
...
int InitBuffer(void)
{
   ...
   BlockBufferCreate(2, sizeof(common_big_package_t), "network-udp-common-big-recv");
	...
	ret = BlockBufferCreate(3, 64, "rs485-recv-cmd-buffer");
	if ( 0 == ret )
	{
		return -30;
	}

	ret = BlockBufferCreate(3, 32, "rs485-send-cmd-buffer");
	if ( 0 == ret )
	{
		return -31;
	}

	ret = BlockBufferCreate(3, sizeof(common_package_t), "monitro_control_info_recv_buffer");
	if ( 0 == ret )
	{
		return -32;
	}

	ret = BlockBufferCreate(3, sizeof(common_package_t), "monitro_control_info_send_buffer");
	if ( 0 == ret )
	{
		return -33;
	}
...
void RegisterCommonBufferRecvBuffer()
{
	...
	ZhwBufferRegister(1,COMMON_PACKAGE_TYPE_TCMS_CMU_STATE,1, bcu_state.bcu_recv_tcms_cmu_state_buffer_id);
	int ret = ZhwBufferRegister(1,COMMON_PACKAGE_TYPE_RUNNING_MODE_INFO,1, bcu_state.bcu_recv_monitor_control_info_id);
	ret = ZhwBufferRegister(1,COMMON_PACKAGE_TYPE_VOLUME_ADJ_INFO,1, bcu_state.bcu_recv_monitor_control_info_id);
	diag_printf("ret %d\n",ret);
	ZhwBufferRegister(1,COMMON_PACKAGE_TYPE_PCU_CONTROL,1, bcu_state.bcu_recv_monitor_control_info_id);
} 
...
void BCUControlMainBufferInit()
{
	...
	bcu_state.rs485_cmd_recv_buffer_id = BlockBufferOpen("rs485-recv-cmd-buffer");
	bcu_state.rs485_cmd_send_buffer_id = BlockBufferOpen("rs485-send-cmd-buffer");

	bcu_state.bcu_recv_monitor_control_info_id = BlockBufferOpen("monitro_control_info_recv_buffer");
}
...
{///<判断D2D是否已经完成
	if(0 && bcu_state.d2d_is_connecting_without_ts == 2)  ///< wait for d2d connect
	{
		return;  
	}
...
//			UpdataGlobalDeviceInfo(d2d_over_package);/*Update global device information*/
			...
			if(bcu_state.bcu_active_intercom_state->state_id == D2D_HANGUP_D2P_EVENT)//pending
			{
				...
				BcuSetRs485State(D2D_INTERCOMM_EXIT);
			...
			else if(bcu_state.bcu_active_intercom_state->state_id == D2D_INTERCOMM_EVENT)//idle
			{
				...
				diag_printf("bcu_state.bcu_active_intercom_state->state_id-%d\n",bcu_state.bcu_active_intercom_state->state_id);
				BcuSetRs485State(D2D_INTERCOMM_EXIT);
			}
...
{///<网络结束D2D
	   ...
	   bcu_state.bcu_active_intercom_state->state_id == D2D_HANGUP_D2P_EVENT)
	{
		...
		else if(bcu_state.bcu_active_intercom_state->state_id == D2D_INTERCOMM_EVENT)//idle
		{
			...
			debug_print2(("We will finish the D2D\n"));
...
{
	if(bcu_state.opposite_live_ann_active == 1) 
	{
		return;
	}
	...
	{
		debug_print2(("I want to enter D2D state\n"));

		serial_info.serial_data.func_code = WRITE_REG_FUNC;
		serial_info.serial_data.start_addr = 0x0084;
		serial_info.serial_data.reg_count = 0x0001;
		serial_info.serial_data.byte_count = serial_info.serial_data.reg_count*2;
		serial_info.serial_data.reg_value = 0x0001;
		CreateInfoFrame(serial_info.serial_data);
		...
		bcu_state.d2d_is_connecting_without_ts = 2;
		BcuSetRs485State(D2D_INTERCOMM_REQUEST);
	}
	   ...
	   param_network_cmd_info->send_information.event_info_intercom.d2d_intercomm.d2d_intercomm_response == 0)
	{
		...
		if(bcu_state.bcu_active_intercom_state->state_id == D2P_INTERCOMM_EVENT)	/*There have driver request*/
		{
			...
			if(WhetherWantToExchangeState(param_network_cmd_info->send_information))//系统转换成司机对讲状态，并且使的PCU进入Pending状态
			{
				...
				diag_printf("mmmmmmmmmmmm d2d_hangup_d2p_flag-1\n");
				bcu_state.d2d_hangup_d2p_flag = 1;
				...
				cyg_thread_delay(100);
				BcuSetRs485State( D2D_INTERCOMM_ENTER );
			}
			else
			{
...
void TouchScreenShowRegionInfo(void)
{
	char stationname[STATION_NAME_LENGTH];
	station_info_t *pstation_info;
	unsigned char station_no;
	//diag_printf("$$$$ start_station_no %d\n",bcu_state.bcu_region_info[1].p_regin_info->start_station_no);
	//diag_printf("$$$$ over_station_no %d\n",bcu_state.bcu_region_info[1].p_regin_info->over_station_no);	
	pstation_info = bcu_state.pp_station_info[1];

	if(bcu_state.bcu_region_info[1].p_regin_info->running_direction == 1)
	{
		station_no = bcu_state.bcu_region_info[1].p_regin_info->start_station_no;
		start_value->label("火车北站");
		start_value->redraw();
		station_no = bcu_state.bcu_region_info[1].p_regin_info->over_station_no;
		end_value->label("北站西二路");
		end_value->redraw();
	}else if(bcu_state.bcu_region_info[1].p_regin_info->running_direction == 2)
	{
		station_no = bcu_state.bcu_region_info[1].p_regin_info->start_station_no;
		start_value->label("北站西二路");
		start_value->redraw();
		station_no = bcu_state.bcu_region_info[1].p_regin_info->over_station_no;
		end_value->label("火车北站");
		end_value->redraw();
	}
}
...
void TouchScreenShowSatationInfo(void)
{
	station_info_t *pstation_info;
	pstation_info = bcu_state.pp_station_info[1];

	current_value->label((char *)pstation_info[station_info_from_network.current_station_code].name);
	current_value->redraw();
	
	next_value->label((char *)pstation_info[station_info_from_network.next_station_code].name);
	next_value->redraw();
	if(station_info_from_network.door_side == 0)
	{
		open_value->label((char *)"左侧");
		open_value->redraw();
	}
	else if(station_info_from_network.door_side == 1)
	{
		open_value->label((char *)"右侧");
		open_value->redraw();
	}
...
void CreateInfoFrame(serial_data_t serial_data)
{
	int send_count = 0;
	char tmp_buf[32];
	...
	tmp_buf[send_count++] = RTU_ADDR_BYTE;
	tmp_buf[send_count++] = serial_info.serial_data.func_code;
	tmp_buf[send_count++] = (serial_info.serial_data.start_addr>>8) & 0xff;
	tmp_buf[send_count++] = serial_info.serial_data.start_addr & 0xff;
	tmp_buf[send_count++] = (serial_info.serial_data.reg_count>>8) & 0xff;
	tmp_buf[send_count++] = serial_info.serial_data.reg_count & 0xff;
	tmp_buf[send_count++] = serial_info.serial_data.byte_count;
	tmp_buf[send_count++] = (serial_info.serial_data.reg_value>>8) & 0xff;
	tmp_buf[send_count++] = serial_info.serial_data.reg_value & 0xff;
	serial_info.serial_data.crc16 = CRC16(tmp_buf,send_count);
	tmp_buf[send_count++] = serial_info.serial_data.crc16 & 0xff;
	tmp_buf[send_count++] = (serial_info.serial_data.crc16>>8) & 0xff;
	serial_info.send_count = send_count;
}
...
void MRHandle()
{
	...
	{
		SendNoPCUToCCU();
		BcuSetRs485State(RESET_ALL_DEV);
	}
}

void do_timeout(cyg_handle_t alarm, cyg_addrword_t data)
{
    int i;
    cyg_int32 min_delta;
...
cyg_int32 timeout(cyg_int32 delta, timeout_fun *fun, void *arg)
{
    int i;
    cyg_int32 min_delta;
...
void untimeout(unsigned int stamp)
{
    if (stamp != 0) {
...
void BcuSetRs485State(char status)
{
		...
		if(bcu_state.current_pa_running_mode == DIGTAL_MODE)
		{
			if(status == LIVE_ANN_EXIT || status == LIVE_ANN_ENTER )
			return;
		}

		if(bcu_state.current_d2d_running_mode == DIGTAL_MODE)
		{
			if(status == D2D_INTERCOMM_EXIT 
				|| status == D2D_INTERCOMM_ENTER || status == D2D_INTERCOMM_REQUEST)
			return;
		}

		serial_info.cur_status = status;

		if(status != READ_ALL_REGS)
		    debug_print2(("##system control set status %s\n",state_str[status]));

		if(status == LIVE_ANN_EXIT) {
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x10;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x81;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x01;	
			tmp_buf[send_count++] = 0x02;
			tmp_buf[send_count++] = 0x00;	
			tmp_buf[send_count++] = 0x01;
			unsigned short crc_val = CRC16(tmp_buf, send_count);
			tmp_buf[send_count++] = (unsigned char)((crc_val)&0xff);		
			tmp_buf[send_count++] = (unsigned char)((crc_val)>>8);
			serial_info.send_count = send_count;
			
		} else if (status == LIVE_ANN_ENTER) {	
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x10;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x80;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x01;	
			tmp_buf[send_count++] = 0x02;
			tmp_buf[send_count++] = 0x00;	
		...
		else if(status == D2D_INTERCOMM_EXIT){
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x10;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x86;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x02;
			tmp_buf[send_count++] = 0x00;		
		...
		else if(status == D2D_INTERCOMM_ENTER){
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x10;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x85;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x02;
			tmp_buf[send_count++] = 0x00;		
		...
		else if(status == D2D_INTERCOMM_REQUEST){
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x10;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x84;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x02;
			tmp_buf[send_count++] = 0x00;		
		...
		else if(status == D2P_INTERCOMM_EXIT){
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x10;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x83;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x02;
			tmp_buf[send_count++] = 0x00;		
		...
		else if(status == D2P_INTERCOMM_ENTER)
		{
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x10;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x82;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x02;
			tmp_buf[send_count++] = 0x00;		
		...
		else if(status == D2P_ANY_INTERCOMM_ENTER)
		{
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x10;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x8B;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x02;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = bcu_state.pcu_request_info.recept_pcu_no +100;
			unsigned short crc_val = CRC16(tmp_buf, send_count);
			tmp_buf[send_count++] = (unsigned char)((crc_val)&0xff);
			tmp_buf[send_count++] = (unsigned char)((crc_val)>>8);
			serial_info.send_count = send_count;
		...
		else if ( status == READ_ALL_REGS ) {
			///< ask dot-table
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x03;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x13;
			unsigned short crc_val = CRC16(tmp_buf, send_count);
			tmp_buf[send_count++] = (unsigned char)((crc_val)&0xff);		
			tmp_buf[send_count++] = (unsigned char)((crc_val)>>8); 	
			serial_info.send_count = send_count;
		}
		else if ( status == SET_LOCAL_PTT ) {
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x10;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x88;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x02;
			tmp_buf[send_count++] = 0x00;		
		...
		else if ( status == SET_REMOTE_PTT ) {
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x10;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x89;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x02;
			tmp_buf[send_count++] = 0x00;		
		...
		else if ( status == RESET_ALL_DEV ) {
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x10;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x8A;
			tmp_buf[send_count++] = 0x00;
			tmp_buf[send_count++] = 0x01;
			tmp_buf[send_count++] = 0x02;
			tmp_buf[send_count++] = 0x00;
		...
		BlockBufferWrite(bcu_state.rs485_cmd_send_buffer_id, tmp_buf, serial_info.send_count);
}

void SetMultiPCUquest(unsigned char number)
{
	network_send_package_t  simu_network_package;
	strcpy(simu_network_package.send_information.src_devices_name, "PCU");
	simu_network_package.send_information.src_devices_no =  number;
	simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_active = 1;
	simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_request_or_over = 1;
	simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_response = 0;
	simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_whether_is_connecting = 0;
	simu_network_package.send_information.event_type_intercom = D2P_INTERCOMM_EVENT;
	simu_network_package.send_information.event_type_ann = ANN_IDLE;
	BlockBufferWrite(bcu_state.cmd_recv_buffer_id, &simu_network_package, sizeof(simu_network_package));
	strcpy(simu_network_package.dst_devices_name, "CCU");
	simu_network_package.dst_devices_no = 1;
	BlockBufferWrite(bcu_state.cmd_send_buffer_id, &simu_network_package, sizeof(simu_network_package));
	strcpy(simu_network_package.dst_devices_name, "CCU");
	simu_network_package.dst_devices_no = 2;
	BlockBufferWrite(bcu_state.cmd_send_buffer_id, &simu_network_package, sizeof(simu_network_package));
}



void SetPCURequestAccordingToRs485()
{
	network_send_package_t  simu_network_package;
	strcpy(simu_network_package.send_information.src_devices_name, "PCU");
	simu_network_package.send_information.src_devices_no =  bcu_state.pcu_request_info.current_request_pcu_no;
	simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_active = 1;
	simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_request_or_over = 1;
	simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_response = 0;
	simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_whether_is_connecting = 0;
...
void SetPCUConnectAccordingToRs485()
{
	network_send_package_t  simu_network_package; 
	strcpy(simu_network_package.send_information.src_devices_name, "PCU");
	simu_network_package.send_information.src_devices_no = bcu_state.pcu_request_info.current_conect_pcu_no;
	simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_active = 1;	
	simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_whether_is_connecting = 1;							
	                 simu_network_package.send_information.event_type_intercom = D2P_INTERCOMM_EVENT;
	simu_network_package.send_information.event_type_ann = ANN_IDLE;
	strcpy(simu_network_package.dst_devices_name, "CCU");
	simu_network_package.dst_devices_no = 1;
...
void SetAnyPCUAccordingToCCU(unsigned char *pcu_request_array)
{
...
//		return;
	...
	if((bcu_state.pcu_request_info.recept_pcu_no != 0xff || bcu_state.pcu_request_info.refuse_pcu_no == 0xff) || bcu_state.d2p_button_state == 0)
	{
		int i;
		for(i = 0; i< 12; i++)
		{
			if(bcu_state.pcu_request_info.recept_pcu_no == pcu_request_array[i])
			{
				bcu_state.pcu_any_recept_state = 1;
				diag_printf("SetAnyPCUAccordingToCCU and pcu request ok !\n");
				network_send_package_t  simu_network_package;
				simu_network_package.send_information.event_type_ann = 254;
				simu_network_package.send_information.event_type_intercom = D2P_INTERCOMM_EVENT;
				simu_network_package.send_information.event_type_ann = ANN_IDLE;
				strcpy(simu_network_package.send_information.src_devices_name, "BCU");
				simu_network_package.send_information.src_devices_no=bcu_state.bcu_info.devices_no;
				simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_pcu_device_no = bcu_state.pcu_request_info.recept_pcu_no;
				simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_bcu_device_no = bcu_state.bcu_info.devices_no;
				simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_active = 1;
				simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_request_or_over = 0;
				...
				bcu_state.bcu_active_intercom_state->enter(&(simu_network_package.send_information));
				bcu_state.d2p_button_state =1 ;
				break;
			}
		}
	}
	else if((bcu_state.pcu_request_info.refuse_pcu_no == 0 || bcu_state.pcu_request_info.recept_pcu_no == 0xff) && bcu_state.d2p_button_state == 1)
	{
		bcu_state.pcu_request_info.refuse_pcu_no = bcu_state.pcu_request_info.recept_pcu_no;
		bcu_state.pcu_any_recept_state = 0;
		diag_printf("SetAnyPCUAccordingToCCU and pcu refuse ok !\n");
		network_send_package_t  a;
		diag_printf("minotor refuse multi test pcu\n");
		...
		a.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_pcu_device_no = bcu_state.pcu_request_info.refuse_pcu_no;
		...
		strcpy(a.dst_devices_name, "CCU");
		a.dst_devices_no = 1;
		BlockBufferWrite(bcu_state.cmd_send_buffer_id, &a, sizeof(a));
		strcpy(a.dst_devices_name, "CCU");
		a.dst_devices_no = 2;
		BlockBufferWrite(bcu_state.cmd_send_buffer_id, &a, sizeof(a));
...
void SetPCUClearAccordingToRs485()
{
	network_send_package_t  simu_network_package; 
	strcpy(simu_network_package.send_information.src_devices_name, "PCU");
	simu_network_package.send_information.event_info_intercom.d2p_intercomm.d2p_intercomm_active = 0;								
	                 simu_network_package.send_information.event_type_intercom = D2P_INTERCOMM_EVENT;
	simu_network_package.send_information.event_type_ann = ANN_IDLE;
	if(bcu_state.pcu_request_info.current_conect_pcu_no != 0xff)
	{
		simu_network_package.send_information.src_devices_no = bcu_state.pcu_request_info.current_conect_pcu_no;
		BlockBufferWrite(bcu_state.cmd_recv_buffer_id, &simu_network_package, sizeof(simu_network_package));
		strcpy(simu_network_package.dst_devices_name, "CCU");
	...
	else if(bcu_state.pcu_request_info.current_request_pcu_no != 0xff)
	{
		simu_network_package.send_information.src_devices_no = bcu_state.pcu_request_info.current_request_pcu_no;
		BlockBufferWrite(bcu_state.cmd_recv_buffer_id, &simu_network_package, sizeof(simu_network_package));
	}
...
void SetD2DRequestAccordingToRs485()
{
	network_send_package_t  simu_network_package;
	strcpy(simu_network_package.send_information.src_devices_name, "BCU");
	simu_network_package.send_information.src_devices_no = bcu_state.opposite_bcu_no;
	simu_network_package.send_information.event_info_intercom.d2d_intercomm.d2d_intercomm_active = 1;
	simu_network_package.send_information.event_info_intercom.d2d_intercomm.d2d_intercomm_request_or_over = 0;
	simu_network_package.send_information.event_info_intercom.d2d_intercomm.d2d_intercomm_response = 0;
	                 simu_network_package.send_information.event_type_intercom = D2D_INTERCOMM_EVENT;
	simu_network_package.send_information.event_type_ann = ANN_IDLE;
	BlockBufferWrite(bcu_state.cmd_recv_buffer_id, &simu_network_package, sizeof(simu_network_package));
}

void FinishD2DAccordingToRs485()
{
	network_send_package_t  simu_network_package;
	strcpy(simu_network_package.send_information.src_devices_name, "BCU");
	simu_network_package.send_information.src_devices_no = bcu_state.opposite_bcu_no;
	simu_network_package.send_information.event_info_intercom.d2d_intercomm.d2d_intercomm_active = 0;
	simu_network_package.send_information.event_info_intercom.d2d_intercomm.d2d_intercomm_request_or_over = 0;
	simu_network_package.send_information.event_info_intercom.d2d_intercomm.d2d_intercomm_response = 0;
	                 simu_network_package.send_information.event_type_intercom = D2D_INTERCOMM_EVENT;
	simu_network_package.send_information.event_type_ann = ANN_IDLE;
...
void Rs485ErrorHandle()
{
	if(serial_info.cur_status == D2D_INTERCOMM_REQUEST)
	{
		debug_print2(("<1223> error happend -%d-\n",serial_info.cur_status));
...
void PCHandle()
{
	...
	int dst_no = 0;
//	if(bcu_state.pcu_any_recept_state == 1)
//	{
//		return;
//	}
	...
	{
		if(bcu_state.bcu_active_intercom_state->state_id == D2P_INTERCOMM_EVENT) ///< two bcu recept pcu in same time 
		{
			bcu_state.bcu_active_intercom_state->exit();
			bcu_state.bcu_active_intercom_state = &idle_intercom_state;
			bcu_state.bcu_active_intercom_state->enter(&(a.send_information));
	...
	if(GetD2PExternButtonState() == 1)
	{
		...
		{
			diag_printf("<1223-1> pcu_device_no-%d\n",dst_no);
			if(bcu_state.bcu_active_ann_state->state_id == 2 &&
				bcu_state.bcu_active_intercom_state->state_id == 255)
			{
				ExitFromLiveWithoutTS();
		...
		{	
			diag_printf("<1223-2> pcu_device_no-%d\n",dst_no);
			a.send_information.event_type_ann = 254;
			a.send_information.event_type_intercom = D2P_INTERCOMM_EVENT;
			strcpy(a.send_information.src_devices_name,"BCU");
			a.send_information.src_devices_no=bcu_state.bcu_info.devices_no;
