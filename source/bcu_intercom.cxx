...
void IdleIntercomEnter(send_infomation_t *send_information_intercomm_idle)
{
	...
	debug_print2(("I am idle intercom enter\n"));
...
//	SetAudioDataDestination("BCU",bcu_state.opposite_bcu_no);
	...
	else
	{
		...
		SetAudioDataDestination("BCU",bcu_state.opposite_bcu_no); ///< for sh17
	}
...
//	MicZeroAfterWaveHeader(bcu_state.audio_data_recv_buffer_id);
	...
	{
		BcuSetRs485State(D2P_ANY_INTERCOMM_ENTER);
	}
	else
	{
		BcuSetRs485State(D2P_INTERCOMM_ENTER);
	}
...
//	AdjustVolumeAfterCODEC();
	...
	if(bcu_state.d2d_hangup_d2p_flag != 1)
	{
		BcuSetRs485State(D2P_INTERCOMM_EXIT);
	}
	else
	{
		bcu_state.d2d_hangup_d2p_flag = 0;
	}
	...
	intercomm_debug_print(("Init, D2pReq, enter: %d, %d, %d--%d \n", GetSndCurrentMode(),
         ...
         if( (bcu_state.pcu_request_info.open_snd_playback==0 && 2==GetSndCurrentMode()) ||
	      (bcu_state.pcu_request_info.pcu_alarm_playing==0 && 1==GetSndCurrentMode()) ||
	       (bcu_state.pcu_request_info.open_snd_playback==2 && 2==GetSndCurrentMode()))
         {
...
{///<在CODEC下调节音量
	diag_printf("AdjustVolumeAfterCODEC = %d,%d\n",current_sounder_mode,bcu_state.device_volume.d2d_volume);
	if(current_sounder_mode != BCU_SND_MODE_CODEC)
	{
		return ;
	}
	switch(bcu_state.device_volume.d2d_volume)
	{
		case 1:AdjustBCUSNDVolume(80);break;
		case 2:AdjustBCUSNDVolume(65);break;
		case 3:AdjustBCUSNDVolume(40);break;
		case 4:AdjustBCUSNDVolume(26);break;
		case 5:AdjustBCUSNDVolume(2);break;
	}
}
...
{
	if(BCUGetDataFromCCUState() == 3)
	{
	//	if(bcu_state.bcu_have_the_initiative_of_cmu == 1)
		{
