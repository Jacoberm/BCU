...
void LiveAnnEnter(send_infomation_t *send_information_live)
{
	...
	BcuSetRs485State(LIVE_ANN_ENTER);

	if(bcu_state.current_pa_running_mode == ANALOG_MODE)
	{
		debug_print2(("<0727> Ann:	Analog\n"));
...
//	CloseSndCard(&bcu_audio_handle,"codec","finish");
    ...
    //BCU_LED_BUTTON1_ON;
	 ...
	 BcuSetRs485State(LIVE_ANN_EXIT);
...
{///<创建同步定时器
	...
	cyg_alarm_initialize(alarm_handle_sync,cyg_current_time()+600,600);
...
void DevSyncHandle()
{
	last_control_flag = control_flag;control_flag = 17;
	if(wherther_send_sync_signal == 1)
	{///<当前是否需要发送同步信号
		last_control_flag = control_flag;control_flag = 18;
		wherther_send_sync_signal = 0;
		SendSYNCPackageToEamp();
		last_control_flag = control_flag;control_flag = 19;
		CloseSYNCTimer();
		last_control_flag = control_flag;control_flag = 20;
	}
	last_control_flag = control_flag;control_flag = 21;
}
